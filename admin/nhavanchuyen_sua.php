<?php
	...
	if(isset($_SESSION['admin_login'])&&$_SESSION['admin_login'] == true){
			$ma = $_GET['id'];
			$dulieu = "SELECT * FROM nhavanchuyen WHERE nvc_ma='$ma'";
			$kq = mysqli_query($con,$dulieu);
			$mangdl = mysqli_fetch_array($kq);
			$ten = $email = $sdt = $diachi = $taikhoan = $matkhau = "";
			$nameErr = $emailErr = $phoneErr = $usernameErr = $passwordErr = $tbloi  = "";
		...
		//xong phần xử lý ảnh
				...
				if($ten != null && $email != null && $sdt != null && $diachi != null) {
					$capnhat = "UPDATE nhavanchuyen SET nvc_ten='$ten',email='$email',sdt='$sdt',diachi='$diachi',anh='$anh' WHERE nvc_ma='$ma'";
					if(mysqli_query($con,$capnhat)) {
						header('Refresh:0;url=nhavanchuyen.php');
					}
					else {
...
<html>
<head>
	<title> Sửa thông tin nhà vận chuyển </title>
	<meta charset="utf-8">
...
<?php include("../includes/menu_admin.php") ?>
		...
		<form action="" method="POST" enctype="multipart/form-data">
			...
			<fieldset class="form-group row">
				...
				<div class="col-8"><input type="text" name="ten" class="form-control" id="a1" required value="<?php echo $mangdl['nvc_ten']; ?>">
